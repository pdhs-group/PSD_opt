...
from lmcann.core.bond_break_post import calc_int_BF
from optframework.utils.func.jit_dpbe_rhs import lam, lam_2d, heaviside
import lmcann.core.ANN_func as model_func
## external package
import math
import random
import pickle
...
class ANN_bond_break():
    def __init__(self,dim,NS,S,global_seed, path_ori_data=None, path_scaler=None, path_train_data=None):
        self.pth = Path(os.getcwd()).resolve()
        if path_ori_data is None:
            self.path_ori_data = os.path.join(self.pth,'simulation_data')
        else: 
            self.path_ori_data = path_ori_data
        if path_scaler is None:
            self.path_scaler = os.path.join(self.pth,'Inputs_scaler.pkl')
        else:
            self.path_scaler = path_scaler
        if path_train_data is None:
            self.path_train_data = os.path.join(self.pth,'output_data_vol.pkl')
        else:
    ...
    def load_all_data(self, path_ori_data):
        files = [os.path.join(path_ori_data, f) for f in os.listdir(path_ori_data) if f.endswith('.npy')]
        if len(files) == 0:
            raise ValueError(f"No data found in the target path: {path_ori_data}")
        for file in files:
            filename = os.path.basename(file)
            parsed_variables = np.array(filename[:-4].split('_'), dtype=float)
            # 使用内存映射方式读取数据，减少内存占用
            F_tem = np.load(file, allow_pickle=True, mmap_mode='r')
            shape = F_tem.shape[0]
            ...
            yield parsed_variables, F_norm_tem
    ...
    def Outputs_on_grid(self, data_gen,
        ...
        for parsed_variables, F_norm_tem in data_gen:

            X1 = parsed_variables[1]
            NO_FRAG = parsed_variables[2]
            if X1 == 1:

                tem_Outputs, prob, x_mass = self.direct_psd_1d(
                    F_norm_tem[:, 0], NO_FRAG, B_c, v1, M1_c, e1, self.x)
                Outputs_1d.append(tem_Outputs)

                tem_Inputs = [parsed_variables[j] for j in (0, 2, 6, 7) if j < len(parsed_variables)]
                Inputs_1d.append(tem_Inputs)
                mean_energy = F_norm_tem[:, 2].mean()
                Outputs_1d_E.append(mean_energy)
                all_prob_list.append(prob)
                all_x_mass_list.append(x_mass)
                all_y_mass_list.append(0)
            else:

                tem_OutputsX1, tem_OutputsX2, prob, x_mass, y_mass = self.direct_psd_2d(
                ...
                Inputs_2d.append(parsed_variables)
                mean_energy = F_norm_tem[:, 2].mean()
                Outputs_2d_E.append(mean_energy)
                all_prob_list.append(prob)
                all_x_mass_list.append(x_mass)
                all_y_mass_list.append(y_mass)
        ...
        Inputs_1d_scaled = scaler_1d.fit_transform(Inputs_1d) if len(Inputs_1d) > 0 else np.array([])
        Inputs_2d_scaled = scaler_2d.fit_transform(Inputs_2d) if len(Inputs_2d) > 0 else np.array([])
    
        all_data = [
            [np.array(Inputs_1d), np.array(Outputs_1d), np.array(Inputs_1d_scaled), np.array(Outputs_1d_E)],
            [np.array(Inputs_2d), np.array(Outputs_2dX1), np.array(Outputs_2dX2), np.array(Inputs_2d_scaled), np.array(Outputs_2d_E)]
        ]
        all_prob = np.array(all_prob_list)
        all_x_mass = np.array(all_x_mass_list)
        all_y_mass = np.array(all_y_mass_list)
    ...
    def direct_psd_2d(self,F_norm, NO_FRAG,X1,B_c,v1,v2,M1_c,M2_c,e1,e2,x,y):
        ...
        tolerance = 1e-3
        if not (1 - tolerance <= x_mass <= 1 + tolerance):
            print("WARNING: x_mass = {:.6f} violates mass conservation!".format(x_mass))
            B_vol_X1 /= x_mass
        if not (1 - tolerance <= y_mass <= 1 + tolerance):
            print("WARNING: y_mass = {:.6f} violates mass conservation!".format(y_mass))
            B_vol_X2 /= y_mass
    ...
    def processing_train_data(self):
        if not os.path.exists(self.path_train_data):
            ...
            data_gen = self.load_all_data(self.path_ori_data)
            all_data, all_prob, all_x_mass, all_y_mass = self.Outputs_on_grid(data_gen,
                                                                             B_c,v1,v2,M1_c,M2_c,e1,e2)
            return all_data, all_prob, all_x_mass, all_y_mass
        else:
            print(f"Processed training data detected{self.path_train_data},If you want to reprocess, please delete this file.")
    ...
    def plot_1d_F(self, epochs_total, data_index=0, vol_dis=False):
        self.check_x_y()
        test_Input = self.all_data[2][data_index].reshape(1,4)
    ...
    def plot_2d_F(self, epochs_total,data_index=0,vol_dis=False):
        self.check_x_y()
        test_Input = self.all_data[3][data_index].reshape(1,8)
        ...
        test_counts = (1e0 * test_Output).astype(int)
        predicted_counts = (1e0 * predicted_Output).astype(int)
        x_test_counts = []
        x_predicted_counts = []
        y_test_counts = []
        y_predicted_counts = []
...
if __name__ == '__main__':
    ...
    # data_dir = '/pfs/work7/workspace/scratch/px2030-2025'
    # path_ori_data = os.path.join(data_dir, 'simulation_data')
    # path_scaler = os.path.join(data_dir, 'ANN_train', 'Inputs_scaler.pkl')
    # path_train_data = os.path.join(data_dir, 'ANN_train', 'output_data_vol.pkl')
