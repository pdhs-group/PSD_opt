...
def generate_dataset():
    ...
    A0 = 1.0
    A_values = [6**2*A0, 8**2*A0, 10**2*A0, 15**2*A0, 20**2*A0, 40**2*A0] 
    NO_FRAG_values = [2,8,16]
    X1_values = [0.1, 0.5, 1]
    ...
    N_GRIDS, N_FRACS = 400, 200
    INIT_BREAK_RANDOM = False
    
    # Make sure there is a directory to save the data
    output_dir = '/pfs/work7/workspace/scratch/px2030-2025/simulation_data'
    os.makedirs(output_dir, exist_ok=True)
    
    tasks = []
    # go through all the combination of parameters
    for A, X1, NO_FRAG, STR1, STR2, STR3,int_bre, gamma in itertools.product(
            A_values, X1_values, NO_FRAG_values, STR_elements, STR_elements, STR_elements, int_bre_values, gamma_values):
        if A/A0 < NO_FRAG:
            continue
        if (X1 == 0 ):
            continue
        ...
        if (STR1 == 1e-3 and STR2 == 1e-3 and STR3 == 1e-3) or (STR1 == 0.5 and STR2 == 0.5 and STR3 == 0.5):
            continue
        args = (A, X1, output_dir, int_bre, STR1, STR2,STR3,NO_FRAG, N_GRIDS, N_FRACS, A0, INIT_BREAK_RANDOM, gamma)
        tasks.append(args)
...
def generate_one_data(args):
    A, X1, output_dir, int_bre, STR1, STR2,STR3,NO_FRAG, N_GRIDS, N_FRACS, A0, INIT_BREAK_RANDOM, gamma = args
    X2 = 1 - X1
    STR = np.array([STR1, STR2, STR3])
    # Optimize MC-Bond-Break Simulation
    F = MC_breakage(A, X1, X2, STR, NO_FRAG, int_bre, N_GRIDS, N_FRACS, 
                    A0, INIT_BREAK_RANDOM, gamma)
    # convert absolute volume to relative volume
    # F[:,0] /= A 
    # construkt the file name
    file_name = f"{A}_{X1}_{NO_FRAG}_{STR1}_{STR2}_{STR3}_{int_bre}_{gamma}.npy"
