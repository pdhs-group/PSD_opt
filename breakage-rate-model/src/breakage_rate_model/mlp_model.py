# -*- coding: utf-8 -*-
"""Lightweight MLP surrogate with a stable full-vector prediction interface."""
...
class _MLPNet(nn.Module):
    def __init__(self, input_dim: int, hidden_sizes: Sequence[int], activation: str):
        super().__init__()
        ...
        for hidden_dim in hidden_sizes:
            layers.extend((nn.Linear(layer_input_dim, hidden_dim), activation_layer()))
...
class MLPEnergyModel(BaseEnergyModel):
    """MLP for ``log(E_need)`` with a fixed external 10-column input contract."""
    ...
    def fit(
        ...
        *,
        ...
        if X_train is None or y_train is None:
            raise ValueError("MLPEnergyModel.fit requires X_train and y_train.")
        ...
        self._net = _MLPNet(self._model_input_dim, self.hidden_sizes, self.activation).to(self.device)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self._x_mean is None or self._x_std is None:
            raise RuntimeError("MLPEnergyModel feature scaler is not initialized.")
