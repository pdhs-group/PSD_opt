...
- one batch NPZ file per parameter group

Output layout:
    <output_dir>/
        pool_index.sqlite
        batches/<group_name>.npz
"""
...
def _init_db(conn: sqlite3.Connection) -> None:
    ...
    conn.executescript(
        ...
        CREATE TABLE groups (
            ...
            batch_npz_relpath TEXT NOT NULL,
...
def convert_h5_pool_to_npz_sqlite_batch(
    ...
    *,
    ...
    batches_root = output_path / "batches"
    ...
    print(f"[NPZ-BATCH] Source HDF5: {input_path}")
    print(f"[NPZ-BATCH] Target dir: {output_path}")
    print(f"[NPZ-BATCH] SQLite: {sqlite_path}")
    print(f"[NPZ-BATCH] datasets={datasets}")
    ...
    try:
        ...
        with h5py.File(input_path, "r") as h5:
            ...
            conn.execute(
                ...
                ("format", json.dumps("npz_sqlite_batch")),
            )
            conn.execute(
                "INSERT INTO meta(key, value_json) VALUES (?, ?)",
            ...
            for group_name in group_names:
                ...
                for sample_name in sample_names:
                    ...
                    for dataset_name in datasets:
                        if dataset_name in sample:
                            batch_payload[f"{sample_name}__{dataset_name}"] = np.asarray(sample[dataset_name][()])

                batch_path = batches_root / f"{group_name}.npz"
                save_npz(batch_path, **batch_payload)
                batch_relpath = batch_path.relative_to(output_path).as_posix()

                conn.execute(
                    "INSERT INTO groups(group_name, np_target, frac_a_target, n_samples, batch_npz_relpath, attrs_json) VALUES (?, ?, ?, ?, ?, ?)",
                    (
                        group_name,
                ...
                print(f"[NPZ-BATCH] group={group_name} samples={len(sample_names)}")
            ...
            print(f"[NPZ-BATCH] Conversion complete. groups={len(group_names)}, samples={total_samples}")
    finally:
        conn.close()
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert an HDF5 aggregate pool into batch-NPZ plus SQLite index."
    )
...
def main() -> None:
    args = _parse_args()
    convert_h5_pool_to_npz_sqlite_batch(
        args.input_h5,
        args.output_dir,
