...
class ReconstructionMixin:
    ...
    recon_monitor_enable: bool = True
    recon_monitor_psd_bins: int = 100
    recon_monitor_grid_log: bool = True
    ...
    def reconstruct(self, method: str = "CAM", reason: str = "", iter_count: Optional[int] = None) -> None:
        ...
        recon_monitor_payload = self._recon_monitor_capture_before(
            method=method,
            reason=reason,
        ...
        self._recon_monitor_capture_after(recon_monitor_payload)
    ...
    def _recon_monitor_ensure_storage(self) -> None:
        if hasattr(self, "recon_monitor_time"):
            return
        self.recon_monitor_time = []
        self.recon_monitor_iter = []
        self.recon_monitor_method = []
        self.recon_monitor_reason = []
        self.recon_monitor_count_before = []
        self.recon_monitor_count_after = []
        self.recon_monitor_M00_ref = []
        self.recon_monitor_M01_ref = []
        self.recon_monitor_M11_ref = []
        self.recon_monitor_M02_ref = []
        self.recon_monitor_M00_post = []
        self.recon_monitor_M01_post = []
        self.recon_monitor_M11_post = []
        self.recon_monitor_M02_post = []
        self.recon_monitor_M00_rel_err = []
        self.recon_monitor_M01_rel_err = []
    ...
    def _recon_monitor_capture_after(self, payload: Optional[Dict[str, object]]) -> None:
        ...
        self._recon_monitor_ensure_storage()
        self.recon_monitor_time.append(float(payload["time_value"]))
        self.recon_monitor_iter.append(int(payload["iter_count"]))
        self.recon_monitor_method.append(str(payload["method"]))
    ...
    def _recon_monitor_build_grid(
        self,
        Vcomp_pre: np.ndarray,
        Vcomp_post: np.ndarray,
    ) -> tuple[np.ndarray, np.ndarray]:
        bins = max(2, int(getattr(self, "recon_monitor_psd_bins", 100)))
        use_log = bool(getattr(self, "recon_monitor_grid_log", True))
        combined_x = np.concatenate([np.asarray(Vcomp_pre[0], dtype=float), np.asarray(Vcomp_post[0], dtype=float)])
        combined_y = np.concatenate([np.asarray(Vcomp_pre[1], dtype=float), np.asarray(Vcomp_post[1], dtype=float)])

        def _make_edges(values: np.ndarray) -> np.ndarray:
            ...
            if vv_nonneg.size == 0:
                return np.linspace(0.0, 1.0, bins + 1)
            vmin = float(np.min(vv_nonneg))
            vmax = float(np.max(vv_nonneg))
            if vmax <= vmin:
                vmax = vmin * (1.0 + 1e-12) + 1e-12
            if use_log:
                vv_pos = vv_nonneg[vv_nonneg > 0.0]
                if vv_pos.size > 0:
                    pmin = float(np.min(vv_pos))
                    pmax = max(float(np.max(vv_pos)), pmin * (1.0 + 1e-12))
                    return np.geomspace(max(pmin, 1e-40), pmax, bins + 1)
            return np.linspace(vmin, vmax, bins + 1)

        return _make_edges(combined_x), _make_edges(combined_y)

    def _recon_monitor_compute_l1(
        self,
        Vcomp_pre: np.ndarray,
        W_pre: np.ndarray,
        Vcomp_post: np.ndarray,
        ...
        x_edges, y_edges = self._recon_monitor_build_grid(Vcomp_pre, Vcomp_post)
        hist_pre, _, _ = np.histogram2d(
            np.asarray(Vcomp_pre[0], dtype=float),
            np.asarray(Vcomp_pre[1], dtype=float),
            bins=[x_edges, y_edges],
            weights=np.asarray(W_pre, dtype=float),
        )
        hist_post, _, _ = np.histogram2d(
            np.asarray(Vcomp_post[0], dtype=float),
            np.asarray(Vcomp_post[1], dtype=float),
            bins=[x_edges, y_edges],
            weights=np.asarray(W_post, dtype=float),
        )
        return float(np.sum(np.abs(hist_post - hist_pre)) / (np.sum(np.abs(hist_pre)) + 1e-40))
# %% CAM
    # =============================
    # CAM grid + assignment
    # =============================
