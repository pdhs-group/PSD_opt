...
class MCPBEBase(BaseSolver):
    ...
    def _init_lmc(self):
        """
        åˆå§‹åŒ– MCPBE ç”¨åˆ°çš„å„ç±» LMC é€‚é…å™¨ï¼š
          - é¢„å¤„ç†/ç¦»çº¿ç±»: table | rank | copula | flow
          - åœ¨çº¿ lmc: live
          - ç ´ç¢ŽçŽ‡ MLP æ¨¡åž‹: breakage_adapter

        è§„åˆ™ï¼š
          1) use_lmc_pre_model=False â†’ ä¸åŠ è½½ä»»ä½•ç¦»çº¿ adapter
          2) use_lmc_pre_model=True â†’ çœ‹ lmc_pre_model é€‰æ‹©å…·ä½“çš„ adapter
          3) live è‹¥ small_particle_policy='fallback'ï¼Œåˆ™å¿…é¡»æœ‰ä¸€ä¸ªç¦»çº¿ adapter å¯ç”¨
          4) è·¯å¾„ç¼ºå¤±æ—¶ç»™å‡ºæ¸…æ™°æŠ¥é”™ï¼›åŒæ—¶ä¼ è¿›å¤šä¸ªè·¯å¾„ä½†å®žé™…æ²¡ç”¨åˆ°çš„ç»™å‡ºè­¦å‘Š
        """
        # -------------- åŸºæœ¬é…ç½®å–å€¼ï¼ˆå…¨éƒ¨æ”¶æŸåˆ° lmc_ å‰ç¼€ï¼‰ --------------
        # æ˜¯å¦ä½¿ç”¨é¢„å¤„ç† LMC
        ...
        # â€”â€” ç ´ç¢ŽçŽ‡ MLP æ¨¡åž‹ç›¸å…³é…ç½® â€”â€” 
        ...
        # è¿™é‡Œä¿å­˜æœ€ç»ˆå®žé™…åˆ›å»ºå‡ºæ¥çš„ adapter
        self.lmc_adapter = None          # ç¦»çº¿ç¢Žç‰‡åˆ†å¸ƒ adapterï¼ˆtable/rank/...ï¼‰
        self.lmc_live = None             # åœ¨çº¿ LMC ç¢Žç‰‡ç”Ÿæˆ
        self.lmc_breakage_adapter = None # åŸºäºŽ MLP çš„ç ´ç¢ŽçŽ‡æ¨¡åž‹

        # -------------- ç¦»çº¿/é¢„å¤„ç†æ¨¡åž‹çš„æž„å»º --------------
        if self.use_lmc_pre_model:
            # ç»Ÿä¸€ä»ŽåŒä¸€ä¸ªåœ°æ–¹ importï¼Œé¿å…å¾ªçŽ¯ import
            ...
            # æ— è®ºå“ªç§ adapterï¼Œéƒ½è®©å®ƒçŸ¥é“å°é¢—ç²’ç­–ç•¥
        ...
        # -------------- åŸºäºŽ MLP çš„ç ´ç¢ŽçŽ‡æ¨¡åž‹åˆå§‹åŒ– --------------
