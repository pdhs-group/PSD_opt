...
class MCPBEBase(BaseSolver):
    ...
    def _init_lmc(self):
        """
        Initialize all LMC-related adapters used by MCPBE:
        ...
        Rules:
          ...
          2) If use_lmc_pre_model=True, the adapter is selected by lmc_pre_model.
          3) Missing required paths raise clear exceptions.
          4) Adapters are imported only when the corresponding flags are enabled.
        """
        ...
        if self.use_lmc_pre_model:
            # Import only when needed to avoid unnecessary deps and circular imports
            ...
            if pre == "table":
                if not self.lmc_tables_path:
                    raise ValueError("lmc_pre_model='table' requires lmc_tables_path.")
                ...
                if not self.lmc_rank_tables_path:
                    raise ValueError("lmc_pre_model='rank' requires lmc_rank_tables_path.")
                ...
                if not self.lmc_copula_path:
                    raise ValueError("lmc_pre_model='copula' requires lmc_copula_path.")
                ...
                if not self.lmc_flow_pure_path or not self.lmc_flow_mix_path: 
                    raise ValueError("lmc_pre_model='flow' requires both lmc_flow_pure_path and lmc_flow_mix_path.") 
            ...
            # Set small-particle policy for any offline adapter
        ...
        if self.lmc_use_breakage_model:
            if not self.lmc_breakage_model_path:
                raise ValueError(
                    "lmc_use_breakage_model=True requires lmc_breakage_model_path."
                )
