...
import pbe_core.func.jit_kernel_agg as kernel_agg
import pbe_core.func.jit_kernel_break as kernel_break
...
class MCPBESolver(BaseSolver):
    ...
    def init_calc(self, init_Vc=True, V_flat=None):
        self._reset_params()
        ...
        if V_flat is None:
            # Build initial V_flat: per-component volumes in rows 0..dim-1
            self.V_flat = np.zeros((self.dim + 1, total_cols), dtype=float)
            cnt = 0
            for i in range(self.dim):
                ...
                p = str(self.PGV[i])
                if p == 'mono':
                    self.V_flat[i, cnt:cnt + ai] = self.v[i]
                elif p == 'norm':
                    mu = self.v[i]
                    sig = float(self.SIG[i]) * mu
                    self.V_flat[i, cnt:cnt + ai] = self._rng.normal(mu, sig, ai)
                elif p == 'weibull':
                    self.V_flat[i, cnt:cnt + ai] = self._rng.weibull(2.0, ai) * (self.SIG[i] * self.v[i])
                else:
                    raise ValueError(f"Unsupported PGV[{i}]='{p}'. Use 'mono' | 'norm' | 'weibull'.")
                cnt += ai
            # finalize total volume row
            self.V_flat[-1, :] = np.sum(self.V_flat[:self.dim, :], axis=0)
            # remove any non-positive columns (robustness against ill-posed draws)
            keep = self.V_flat[-1, :] > 0.0
            self.V_flat = self.V_flat[:, keep]
        ...
        if self.BREAKFVAL != 1 and self.BREAKFVAL != 2:
    ...
    def calc_break_rate(self):
        ...
        if self.dim == 1:
            ...
            kernel_break.calc_B_R_2d_flat(self)
            if self.BREAKFVAL == 5:
    ...
    def check_dim_consistency(self):
        ...
        return (
            len(self.x) == dim and
            len(self.PGV) == dim and
            len(self.SIG) == dim and
            len(self.alpha_prim) == dim**2
        )
