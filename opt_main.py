...
def normal_test(Opt, sample_num, data_name, gruppe_num):
    start_time = time.time()
    ...
    Opt.save_as_png(fig_mix, f"Gruppe{gruppe_num}")
    Opt.save_as_png(fig_NM, f"Gruppe{gruppe_num}-NM")
    Opt.save_as_png(fig_M, f"Gruppe{gruppe_num}-M")
...
if __name__ == '__main__':
    ...
    Opt.k.set_comp_para(R_NM=2.9e-7, R_M=2.9e-7*3)
    # Optimize method: 
