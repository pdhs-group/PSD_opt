...
class StudyConfig:
    ...
    data_root: Path
    mixed_aggregate_pool_root: Path
    pure_aggregate_pool_root: Path
    mixed_model_kind: str
    pure_model_kind: str
    mixed_model_path: Path
    pure_model_path: Path
    mixed_model_logV_bounds: tuple[float, float]
    pure_model_logV_bounds: tuple[float, float]
    pure_pool_MAS: float
...
def build_default_config() -> StudyConfig:
    ...
    config = StudyConfig(
        data_root=DATA_ROOT,
        mixed_aggregate_pool_root=MIXED_AGGREGATE_POOL_ROOT,
        pure_aggregate_pool_root=PURE_AGGREGATE_POOL_ROOT,
        mixed_model_kind=MIXED_BREAKAGE_MODEL_KIND,
        pure_model_kind=PURE_BREAKAGE_MODEL_KIND,
        mixed_model_path=MIXED_BREAKAGE_MODEL_PATH,
        pure_model_path=PURE_BREAKAGE_MODEL_PATH,
        mixed_model_logV_bounds=MIXED_MODEL_LOGV_BOUNDS,
        pure_model_logV_bounds=PURE_MODEL_LOGV_BOUNDS,
        pure_pool_MAS=PURE_POOL_MAS,
...
def apply_case_to_solver(solver: MCPBESolver, config: StudyConfig, case: ScanCase) -> None:
    ...
    solver.lmc_mixed_breakage_model_logV_bounds = config.mixed_model_logV_bounds
    solver.lmc_pure_breakage_model_logV_bounds = config.pure_model_logV_bounds
...
def classify_parameter_changes(changed_names: Iterable[str]) -> RefreshRequirements:
    ...
    lmc_inputs = {
        "MAS", "gamma", "STR", "Df", "NO_FRAG", "int_bre", "lmc_mixed_pool_dir",
        "lmc_pure_pool_dir", "lmc_mixed_MAS", "lmc_pure_MAS", "lmc_A0_runtime",
        "mixed_model_kind", "pure_model_kind", "mixed_model_path", "pure_model_path",
        "mixed_model_logV_bounds", "pure_model_logV_bounds",
...
def refresh_initialized_solver(
    ...
    *,
    ...
    if requirements.rebuild_lmc:
        if solver.lmc_live is not None:
...
def _validate_config_values(config: StudyConfig, cases: Sequence[ScanCase]) -> None:
    for label, kind in (("mixed", config.mixed_model_kind), ("pure", config.pure_model_kind)):
        if kind not in _MODEL_TYPES:
            raise ValueError(f"{label}_model_kind must be one of {tuple(_MODEL_TYPES)}, got {kind!r}.")
    ...
    for label, bounds_value in (("mixed", config.mixed_model_logV_bounds), ("pure", config.pure_model_logV_bounds)):
        ...
        if logV_bounds.shape != (2,) or not np.all(np.isfinite(logV_bounds)) or logV_bounds[0] >= logV_bounds[1]:
            raise ValueError(f"{label}_model_logV_bounds must be two finite increasing log(V/A0) values.")
    initial_logV = math.log(config.initial_particle_volume / config.lmc_A0_runtime)
    mixed_logV_bounds = np.asarray(config.mixed_model_logV_bounds, dtype=float)
    if initial_logV < mixed_logV_bounds[0] or initial_logV > mixed_logV_bounds[1]:
        raise ValueError(
            "The initial mixed normalized particle volume lies outside mixed_model_logV_bounds: "
            f"log(V/A0)={initial_logV:.6g}, bounds={tuple(mixed_logV_bounds)}."
        )
    if not np.isfinite(config.pure_pool_MAS):
        raise ValueError("pure_pool_MAS must be finite.")
...
def _validate_assets(config: StudyConfig) -> None:
    for label, path, root, kind, strength_mode, active_features in (
        ("mixed", config.mixed_model_path, config.mixed_aggregate_pool_root, config.mixed_model_kind, "geometric_mean_relative", None),
        ("pure", config.pure_model_path, config.pure_aggregate_pool_root, config.pure_model_kind, "none", ("logV", "log_gamma")),
    ):
        if not path.is_file():
        ...
        model = BaseEnergyModel.load(str(path), device="cpu")
        expected_type = _MODEL_TYPES[kind]
        if not isinstance(model, expected_type):
            raise TypeError(f"{label}_model_kind={kind!r} requires {expected_type.__name__}, got {type(model).__name__}.")
        if not model.is_fitted:
            raise RuntimeError(f"The selected {label} energy model is not fitted.")
        if model.strength_normalization != strength_mode:
            raise ValueError(f"{label} energy model must use strength_normalization={strength_mode!r}.")
        if active_features is not None and tuple(model.active_feature_names) != active_features:
    ...
    pool = AggPool(str(config.mixed_aggregate_pool_root), warn_out_of_bounds=False)
    try:
        for MAS in config.mas_values:
    ...
    pure_pool = AggPool(str(config.pure_aggregate_pool_root), warn_out_of_bounds=False)
    try:
        cache = pure_pool._get_cache(config.Df, config.pure_pool_MAS)
        A_min, A_max = float(cache["Np_vals"][0]), float(cache["Np_vals"][-1])
        X_values = np.asarray(cache["XA_vals"], dtype=float)
        if normalized_volume < A_min or normalized_volume > A_max:
            raise ValueError(
                f"Initial A/A0={normalized_volume} is outside pure-pool coverage [{A_min}, {A_max}] "
                f"for Df={config.Df}, MAS={config.pure_pool_MAS}."
            )
        if X_values.shape != (1,) or X_values[0] != 0.0:
            raise ValueError("Pure aggregate pool must contain exactly the X1=0 lookup coordinate.")
    finally:
        pure_pool.close_pool_cache()
