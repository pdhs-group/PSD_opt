...
def calc_delta_test(var_delta=False):
    ...
    if var_delta:
        ...
        for start_step in range(1,len(opt.core.t_vec)):
            ...
            delta_arr[start_step] = opt.core.calc_delta(opt.pop_params, x_uni_exp, data_exp)
        return delta_arr
    else:
        delta = opt.core.calc_delta(opt.pop_params, x_uni_exp, data_exp)
        # x_uni, Q3 = return_pop_distribution()
...
if __name__ == '__main__':
    ...
    config_path = r"C:\Users\px2030\Code\PSD_opt\tests\config\opt_config_mcpbe.py"
    opt = OptBase(config_path=config_path)
