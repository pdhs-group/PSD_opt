...
class OptCoreMulti(OptCore):
    ...
    def calc_delta(self, params_in,x_uni_exp, data_exp): 
        ...
        ## for DEBUG
        # if len(self.t_vec) != len(self.extruder.t_vec):
        #     print(params)
        #     print(self.extruder.t_vec)
        
        if self.extruder.calc_status:
            delta = self.calc_delta_tem(x_uni_exp[0], data_exp[0], self.p, self.extruder)
        else:
            print('p not converged')
            delta = 10
            
        if self.extruder_NM.calc_status:
            delta_NM = self.calc_delta_tem(x_uni_exp[1], data_exp[1], self.p_NM, self.extruder_NM)
        else:
            print('p_NM not converged')
            delta_NM = 10
            
        if self.extruder_M.calc_status:    
            delta_M = self.calc_delta_tem(x_uni_exp[2], data_exp[2], self.p_M, self.extruder_M)
        else:
    ...
    def calc_all_pop(self, params=None, t_vec=None):
        ...
        """
        self.calc_pop(self.extruder_NM, params, t_vec, self.init_N_NM)
        self.calc_pop(self.extruder_M, params, t_vec, self.init_N_M)
        self.calc_pop(self.extruder, params, t_vec, self.init_N_2D)           
