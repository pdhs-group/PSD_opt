...
def create_1d_pop(self, t_vec, disc='geo'):
    ...
    None
    """
    self.extruder_NM = ExtruderPBESolver(dim=1,NC=self.NC, disc=disc, t_vec=t_vec, load_attr=False)
    self.extruder_M = ExtruderPBESolver(dim=1,NC=self.NC, disc=disc, t_vec=t_vec, load_attr=False)
    self.p_NM = self.extruder_NM.p
    self.p_M = self.extruder_M.p
...
def calc_pop(self, pop, params=None, t_vec=None, init_N=None):
    ...
    pop.solve_extruder(t_vec=t_vec)      
...
def set_init_pop_para(self,pop_params):
    ...
    None
    """
    self.set_pop_para(self.extruder, pop_params)
    
    if hasattr(self, 'extruder_NM'):
        self.set_pop_para(self.extruder_NM, pop_params)
    if hasattr(self, 'extruder_M'):
        self.set_pop_para(self.extruder_M, pop_params)
...
def set_pop_para(self, pop, params_in):
    ...
    if params_in is None:
        print("Warning: PBE parameter input not detected.")
        return
    
    if not self.set_init_pop_para_flag:
        params_full = params_in.copy()
        params = params_full['pbe_params']['global']
    else:
    ...
    if self.dim == 1:
        ...
        if 'alpha_prim' in params:
            ...
            if params['alpha_prim'].ndim != 0:
                params['alpha_prim'] = np.array([params['alpha_prim'][0]])
    ...
    # Handle alpha_prim and corr_agg for 2D populations
        ...
        if 'alpha_prim' in params:
            ...
            if pop is self.extruder:
                ...
                params['alpha_prim'] = alpha_prim_temp
            elif pop is self.extruder_NM:
                params['alpha_prim'] = alpha_prim_value[0]
            elif pop is self.extruder_M:
                params['alpha_prim'] = alpha_prim_value[2]
        if 'pl_P3' in params and 'pl_P4' in params:
            if pop is self.p_M:
                params['pl_P1'] = params['pl_P3']
                params['pl_P2'] = params['pl_P4']
    if not self.set_init_pop_para_flag:
        pop.get_all_comp_params_from_dict(params_dict=params_full, N_feed=None)
    else:
        pop.get_all_comp_params_from_dict(new_global=params, N_feed=None)
