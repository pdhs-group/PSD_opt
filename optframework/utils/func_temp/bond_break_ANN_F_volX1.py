...
def Outputs_on_grid(path_all_data, all_Inputs, F_norm, data_num,B_c,v1,v2,M1_c,M2_c,e1,e2,x,y):
    ...
    for i in range(data_num):
        ...
        if X1 == 1:
            ...
            tem_Outputs, all_prob[i], all_x_mass[i]= direct_psd_2d(F_norm_tem,NO_FRAG,B_c,v1,v2,M1_c,M2_c,e1,e2,x,y)   
...
def direct_psd_2d(F_norm, NO_FRAG,B_c,v1,v2,M1_c,M2_c,e1,e2,x,y):
    ...
    # B /= NO_FRAG 
    prob = B.sum()
    B_vol = B * np.outer(x[:-1], np.ones(30))
...
def train_and_evaluate_model(model, model_name, train_data, test_data, epochs, x, y=None):
    # train model
    if use_custom_loss:
        train_model(model, train_data[0], train_data[1], epochs=epochs, x=x)
    else:
        model.fit(train_data[0], train_data[1], epochs=epochs, batch_size=32, validation_split=0.2)   
  
    model.save(model_name + '.keras')
    # evaluate_model
    test_res = predictions_test(model_name + '.keras', test_data[0], test_data[1], x=x)
    return test_res

def predictions_test(model_name,test_all_Inputs, test_all_Outputs, x): 
    ...
    for i in range(length):
        pre_all_prob[i] = np.sum(predicted_all_Outputs[i,:]/ np.outer(x, np.ones(30)))
        pre_all_x_mass[i] = np.sum(predicted_all_Outputs[i,:])
    ...
    return mse, mae, mean_all_prob, mean_all_x_mass
...
def plot_error(results,epochs,num_training):
    ...
    ax4, fig4 = pt.plot_data(epochs_array, results['model_2d']['mean_x_mass_erro'],
                           xlbl='Epochs of Model Training / $-$',
                           ylbl='Results of Validation / $-$',
