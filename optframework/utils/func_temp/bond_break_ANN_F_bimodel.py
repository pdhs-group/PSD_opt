...
def generate_grid():
    ...
    v1 = np.zeros(NS+1)
    M1_c = np.zeros(NS+1)
    return B_c,v1,M1_c,e1,x

def Outputs_on_grid(path_all_data, all_Inputs, F_norm, data_num,B_c,v1,M1_c,e1,x):
    Inputs_2d = []
    Inputs_1d = []
    Outputs_2d = []
    Outputs_2dX1 = []
    ...
    for i in range(data_num):
        ...
        if X1 == 1:
            ...
            tem_Outputs, tem_OutputsX1, all_prob[i], all_x_mass[i] = direct_psd_2d(F_norm_tem,NO_FRAG,B_c,v1,M1_c,e1,x)   
    ...
    all_data = [[np.array(Inputs_1d),np.array(Outputs_1d)],
                ...
                [np.array(Inputs_2d),np.array(Outputs_2dX1)]]
    
    with open(path_all_data, 'wb') as file:
        pickle.dump((all_data, all_prob, all_x_mass), file)    
        
    return all_data, all_prob, all_x_mass

def direct_psd_1d(F_norm,NO_FRAG,B_c_tem,v1_tem,M1_c,e1,x,N_test=None):
    B = np.zeros(NS)
    if N_test is None:
        B_c_tem[:-1], M1_c[:-1], _ = calc_int_BF(N_GRIDS * N_FRACS * NO_FRAG, F_norm, e1)
    else:
        B_c_tem[:-1], M1_c[:-1], _ = calc_int_BF(N_test, F_norm, e1)
...
def direct_psd_2d(F_norm, NO_FRAG,B_c,v1,M1_c,e1,x):
    F_V = F_norm[:,0] + F_norm[:,1]
    B = np.zeros(NS)
    B_c[:-1], M1_c[:-1], _ = calc_int_BF(N_GRIDS * N_FRACS * NO_FRAG, F_V, e1)
    v1[B_c != 0] = M1_c[B_c != 0]/B_c[B_c != 0]
    for i in range(NS):
        # Add contribution from LEFT cell (if existent)
        B[i] += B_c[i]*lam(v1[i], x, i, 'm')*heaviside(x[i]-v1[i],0.5)
        # Left Cell, right half
    ...
    B_X1 = np.zeros((NS,NS))
    bin_indices  = np.digitize(F_V, e1) - 1
    X1_in_bins = [[] for _ in range(len(e1) - 1)]
    for idx, bin_index in enumerate(bin_indices):
        if 0 <= bin_index < len(X1_in_bins):
            X1_in_bins[bin_index].append(F_norm[idx, 0])
    
    for i, X1 in enumerate(X1_in_bins):
        B_c_X1 = np.zeros(NS+1)-1
        M_c_X1 = np.zeros(NS+1)-1
        B_c_X1[:-1], M_c_X1[:-1],_ = calc_int_BF(len(X1), X1, e1)
        B_X1[i,:],_,_ = direct_psd_1d(X1, NO_FRAG, B_c_X1, v1, M_c_X1, e1, x)
            
    return B, B_X1, prob, x_mass
...
if __name__ == '__main__':
    ...
    ## 2d model has seven input parameters: Volume, X1, NO_FRAG, STR1, STR2, STR,3, int_bre
    psd = 'direct'
    int_method = 'MC'
    directory = '../../tests/simulation_data'
    test_directory = '../../tests/test_data'
    path_all_data = 'output_data_bi.pkl'
    path_all_test = 'test_data_bi.pkl'
    NS = 30
    S = 2
    ...
    B_c,v1,M1_c,e1,x = generate_grid()
    all_data, all_prob, all_x_mass = Outputs_on_grid(path_all_data, all_Inputs, F_norm, data_num,
                                                                    B_c,v1,M1_c,e1,x)
    test_all_data, _, _ = Outputs_on_grid(path_all_test, test_all_Inputs, test_F_norm, test_data_num,
                                                                    B_c,v1,M1_c,e1,x)
