...
def plot_summary_two_charts(results, n_iter_list, G_flag_list, result_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, "train_test_mean_std_combined.jpg"))
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, "params_normalized_std_combined.jpg"))
    plt.close()
...
def plot_x_50(results, G_flag):
    ...
    for i in range(len(known_params_list)):
        ...
        exp_x50, _ = calc_x50([known_params_list[i]], [data_names_list[i]], init_core=True, opt_params=opt_params_dict[400])
        ax.plot(time_points, exp_x50, 'x', color='black', label='Exp')

        for n_iter in n_iter_list:
            _, mod_x50 = calc_x50([known_params_list[i]], [data_names_list[i]], init_core=False, opt_params=opt_params_dict[n_iter])
...
def main():
    ...
    plot_x_mean(results, "Mean_LocalStirrer")
    # plot_x_50(results, "Mean_LocalStirrer")
...
if __name__ == '__main__':
    ...
    data_dir = "lognormal_curvefit" ## "int1d", "lognormal_curvefit"
    ...
    result_dir = os.path.join(r"C:\Users\px2030\Code\Ergebnisse\Batch_opt\opt_results", "cv_results_group1")
