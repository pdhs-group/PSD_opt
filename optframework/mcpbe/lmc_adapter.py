...
class LMCRankAdapter(LMCBaseAdapter):
    ...
    The .npz file contains, for each (A, X1):
      ...
      - (Optional) two-phase tail composition modeled as Beta(α, β)
...
class LMCLiveAdapter:
    ...
    - Dynamic convergence of NO_FRAG:
        NO_FRAG_eff = min(NO_FRAG, floor(A / A0_run - delta_cells)), but ≥ 2.
    - If A / A0_run < 2:
    ...
    def configure_simulator(
        ...
        *,
        ...
        MAS : float, optional
            Mischgüte (mixing quality) tag used for selecting aggregates.
...
class LMCCopulaAdapter(LMCBaseAdapter):
    ...
    def sample_one_shot(
        ...
        if picked is None:
            # No usable cell found → fallback: equal splitting
        ...
        try:
            ...
            # Any failure inside the cell → fallback: equal splitting
            z = np.full(pick, 1.0 / max(N, 1), dtype=float)
            if pure_A_req:
                rA = z.tolist()
                rB = [0.0] * pick
