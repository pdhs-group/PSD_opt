...
def direct_psd_1d(F_norm,NO_FRAG,B_c_tem,v1_tem,M1_c,e1,x):
    B = np.zeros(NS)
    B_c_tem[:-1], M1_c[:-1], _ = calc_int_BF(N_GRIDS * N_FRACS, F_norm, e1)
...
def direct_psd_2d(F_norm, NO_FRAG,B_c,v1,M1_c,e1,x):
    ...
    B = np.zeros(NS)
    B_c[:-1], M1_c[:-1], _ = calc_int_BF(N_GRIDS * N_FRACS, F_V, e1)
    ...
    for i, X1 in enumerate(X1_in_bins):
        ...
        B_vol_X1[i,:],_,_ = direct_psd_1d(X1, NO_FRAG, B_c_X1, v1, M_c_X1, e1, x)
            
    return B_vol, B_vol_X1, prob, x_mass
